...
# -*- coding: utf-8 -*-
# Seuils Lactate – VMA (v0.8.9)
# - Alerte visuelle + sonore toutes les 5 min (5,10,15,20,25,30)
# - Timer autonome avec auto-refresh
# - Vitesse MLSS liée dynamiquement à SV2 (96%)
# - Graphique MLSS: droites reliant les points, Lactate + FC superposées, fond vert/rouge + badge
# - Suggestion vitesse cohérente
...
def mlss_stability_metrics(t,lac):
    ...
    slope=None; delta=None; stable=False
    if len(t)>=2:
        slope=np.polyfit(t,lac,1)[0]
    try:
        v10=lac[np.where(t==10)[0][0]]; v30=lac[np.where(t==30)[0][0]]
...
with ath_tab:
    ...
    with st.form("ath_form"):
        ...
        if sub: st.success("Fiche enregistrée")

# Analyse Lactate
with lactate_tab:
    show_logo(); st.markdown("### Outil Analyse Lactate")
    st.markdown('https://www.exphyslab.com/lactate')

# MLSS
with mlss_tab:
    show_logo(); st.markdown("### MLSS – Palier 30 min")
    ...
    with col_timer[1]:
        if st.button("⏹️ Arrêter",key="stop_btn"): st.session_state["timer_running"]=False
    with col_timer[2]:
    ...
    if st.session_state.get("timer_running",False) and ("start_time" in st.session_state):
        elapsed=int(time.time()-st.session_state["start_time"]); remaining=max(0,1800-elapsed)
    m,s=divmod(remaining,60)
    st.markdown(f"⏱ Temps restant : **{m:02d}:{s:02d}**")
    if remaining>0 and remaining%300<2:
        st.warning("Alerte : point lactate !"); beep_if_needed(remaining)
    ...
    sv2=st.number_input("SV2 (km/h)",0.0,30.0,st.session_state.get("sv2",0.0),step=0.1,key="sv2",on_change=update_v_target_from_sv2)
    if "v_target" not in st.session_state: update_v_target_from_sv2()
    v_target=st.number_input("Vitesse cible MLSS (km/h)",5.0,30.0,st.session_state["v_target"],step=0.1,key="v_target")
    ...
    with st.form("mlss_form"):
        ...
        if sub_mlss: st.session_state["df_mlss"]=df_mlss_edit
    ...
    fig,ax=plt.subplots(figsize=(9,5)); ax.set_facecolor("#d4edda" if stable else "#f8d7da")
    ...
    if np.isfinite(hr).sum()>=2:
        ax2=ax.twinx(); ax2.plot(t,hr,"s-",color="#d83b01",linewidth=2.0,label="FC (bpm)"); ax2.set_ylabel("FC (bpm)")
        h2,l2=ax2.get_legend_handles_labels(); ax.legend(handles+h2,labels+l2,loc="upper left")
    else: ax.legend(loc="upper left")
    ax.text(0.98,0.04,"● Stable" if stable else "● Instable",transform=ax.transAxes,ha="right",va="bottom",fontsize=11,color="white",bbox=dict(boxstyle="round",facecolor=("#107c10" if stable else "#e81123"),edgecolor="none",alpha=0.9))
    st.pyplot(fig)

    # Suggestion vitesse
    if slope is not None and delta_10_30 is not None:
        step=suggestion_step_from_slope(slope); vt=float(st.session_state["v_target"])
        ...
        if not stable:
            if slope>0.02:
                ...
                suggestion=vt; rationale="Pente quasi nulle"
            st.info(f"Proposition vitesse ajustée : {suggestion:.1f} km/h. {rationale}")
        else: st.success("Lactate stable → conserver vitesse testée")

# Onglet SRS
with srs_tab:
    show_logo(); st.markdown("### Paramétrage SRS")
